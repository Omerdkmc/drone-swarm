...
def battery_test(scf):
    ...
    print("Hovering until disconnect or battery dies...")
    start = time.time()
    try:
        while True:
        ...
        print("Lost connection or error:", e)
    finally:
        elapsed = time.time() - start
        print(f"Flight time until disconnect: {elapsed:.1f} seconds")
...
def position_callback(timestamp, data, logconf):
    ...
    with open('position_log_fullsetpoint_z_largebat.csv', 'a') as f:
        f.write(f'{timestamp},{x},{y},{z}\n')



def set_up_logging(scf):
    _lg_stab = LogConfig(name='Stabilizer', period_in_ms=250)
